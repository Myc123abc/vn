...
public:
  ...
  private:
    ...
    auto get_shader_reflection(IDxcResult* result, ShaderType type) noexcept -> Microsoft::WRL::ComPtr<ID3D12ShaderReflection>;
  ...
  private:
    ...
    std::vector<D3D12_ROOT_PARAMETER1>    _root_params;
    std::queue<D3D12_DESCRIPTOR_RANGE1>   _ranges;
    bool                                  _has_sampler{};
...
private:
  auto compile(std::string_view shader_path, std::string_view include, ShaderType type, std::string_view entry_point) noexcept -> std::pair<Microsoft::WRL::ComPtr<IDxcResult>, Microsoft::WRL::ComPtr<IDxcBlob>>;
