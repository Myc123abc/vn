...
inline void err_if(VkResult res, std::string_view msg) noexcept
{
  err_if(res != VK_SUCCESS, msg);
}

template <typename... T>
inline void err_if(VkResult res, std::format_string<T...> const fmt, T&&... args) noexcept
{
  err_if(res != VK_SUCCESS, fmt, std::forward<T>(args)...);
}
